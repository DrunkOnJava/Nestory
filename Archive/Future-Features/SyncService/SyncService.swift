...
// Import EnvironmentConfiguration for CloudKit container access
@testable import Nestory

// RetryConfig is defined in Infrastructure/Network/HTTPClient.swift
// Services layer can import from Infrastructure per SPEC.json architecture
...
public struct LiveSyncService: SyncService, Sendable {
    private let container: CKContainer
    private let database: CKDatabase
    private let zoneID: CKRecordZone.ID
    private let conflictResolver: any ConflictResolver
    private let logger = Logger(subsystem: EnvironmentConfiguration.shared.bundleIdentifier, category: "SyncService")
    ...
    public init(
        containerIdentifier: String = EnvironmentConfiguration.shared.cloudKitContainer,
        conflictResolver: any ConflictResolver = AutomaticConflictResolver()
    ) {
    ...
    public func pushChanges(_ changes: [SyncChange]) async throws {
        ...
        let records = changes.map { change -> CKRecord in
            ...
            for (key, value) in change.fields {
                record[key] = value as? any CKRecordValue
            }
        ...
        _ = try await withRetry(config: retryConfig) {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            operation.queryResultBlock = { result in
                ...
                case let .success(cursor):
                    continuation.resume(with: .success((fetchedChanges, cursor)))
                case let .failure(error):
                    continuation.resume(with: .failure(SyncError.fetchFailed(error.localizedDescription)))
                }
            }
...
public struct SyncChange: @unchecked Sendable {

...
public final class ShareGroup {
    ...
    public init(
        ...
        // Safe JSON encoding with fallbacks
        do {
    ...
    public var membersList: [Member] {
        ...
        set {
            do {
                ...
                // Log error but don't crash - keep existing data
                print("Warning: Failed to encode members: \(error)")
            }
            updatedAt = Date()
        }
    }
    ...
    public var permissionsSettings: Permissions {
        ...
        set {
            do {
                ...
                // Log error but don't crash - keep existing data
                print("Warning: Failed to encode permissions: \(error)")
            }
            updatedAt = Date()
        }
    }
    ...
    public func generateInviteCode() {
        ...
        let code = (0 ..< 8).compactMap { _ in
            characters.randomElement().map(String.init)
        }.joined()

        // Ensure we got a full 8-character code
        if code.count == 8 {
            inviteCode = code
        } else {
            // Fallback to UUID-based code if randomElement fails

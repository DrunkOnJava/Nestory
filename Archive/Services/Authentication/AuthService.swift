...
public struct LiveAuthService: AuthService, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: EnvironmentConfiguration.shared.bundleIdentifier, category: "AuthService")

    public init(keychain: KeychainStore = KeychainStore(service: "\(EnvironmentConfiguration.shared.bundleIdentifier).auth")) {
    ...
    public func refreshToken(_ token: String) async throws -> AuthCredentials {
        ...
        let newCredentials = AuthCredentials(
            ...
            expiresAt: Date().addingTimeInterval(BusinessConstants.Authentication.accessTokenLifetime),
            provider: .apple,
        )
    ...
    private func createAnonymousCredentials() -> AuthCredentials {
        AuthCredentials(
            ...
            accessToken: generateToken(),
            refreshToken: generateToken(),
            expiresAt: Date().addingTimeInterval(BusinessConstants.Authentication.anonymousSessionLifetime),
            provider: .anonymous,
        )
    }
    ...
    private func createDemoCredentials() -> AuthCredentials {
        AuthCredentials(
            ...
            expiresAt: Date().addingTimeInterval(BusinessConstants.Authentication.demoSessionLifetime),
            provider: .demo,
        )
    }

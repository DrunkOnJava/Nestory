...
public final class PerceptualHash {
    ...
    public func hash(image: UIImage) async throws -> UInt64 {
        try await Task.detached(priority: .userInitiated) {
            let startTime = CFAbsoluteTimeGetCurrent()

            let grayscale = try self.convertToGrayscale(image)
            let resized = try self.resize(grayscale, to: CGSize(width: self.resizeSize, height: self.resizeSize))
            let dctMatrix = try self.performDCT(resized)
            let hash = self.computeHash(from: dctMatrix)

            let duration = CFAbsoluteTimeGetCurrent() - startTime
            self.logger.debug("Computed perceptual hash in \(duration, format: .fixed(precision: 3))s")
    ...
    private func convertToGrayscale(_ image: UIImage) throws -> CIImage {
    ...
    private func performDCT(_ image: CIImage) throws -> [[Float]] {
    ...
    private func computeHash(from dctMatrix: [[Float]]) -> UInt64 {
        var values: [Float] = []

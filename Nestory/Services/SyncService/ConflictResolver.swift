...
public struct AutomaticConflictResolver: ConflictResolver {
    ...
    public func resolve(_ conflicts: [SyncConflict]) async throws -> [ConflictResolution] {
        ...
        for conflict in conflicts {
            ...
            logger.debug("Resolved conflict for \(conflict.recordID) using \(resolution.strategy)")
        }
...
public struct ManualConflictResolver: ConflictResolver {
    ...
    public func resolve(_ conflicts: [SyncConflict]) async throws -> [ConflictResolution] {
        ...
        for conflict in conflicts {
            ...
            resolutions.append(resolution)
            logger.info("User resolved conflict for \(conflict.recordID) using \(strategy)")
        }
...
public struct RuleBasedConflictResolver: ConflictResolver {
    ...
    private let fallbackResolver: ConflictResolver
    private let logger = Logger(subsystem: "com.nestory", category: "RuleBasedResolver")

    public init(
        rules: [Rule],
        fallbackResolver: ConflictResolver = AutomaticConflictResolver()
    ) {
        self.rules = rules
...
public extension RuleBasedConflictResolver.Rule {
    static var preferNewest: Self {
        Rule(field: "updatedAt") { local, remote in
    ...
    static var preferHigherQuantity: Self {
        Rule(field: "quantity") { local, remote in
    ...
    static var preferHigherPrice: Self {
        Rule(field: "purchasePrice") { local, remote in

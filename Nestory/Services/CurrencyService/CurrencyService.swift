...
public struct LiveCurrencyService: CurrencyService {
    private let cache: Cache<String, ExchangeRate>
    private let fileStore: FileStore
    private let httpClient: HTTPClient?
    private let logger = Logger(subsystem: "com.nestory", category: "CurrencyService")
    ...
    public init(httpClient: HTTPClient? = nil) throws {
        ...
        fileStore = try FileStore(directory: "currency")
    ...
    public func updateRates() async throws {
        ...
        do {
            ...
            try await fileStore.save(data, to: "latest_rates.json")
    ...
    public func getHistoricalRate(from: String, to: String, date: Date) async throws -> Decimal? {
        if let data = try? await fileStore.load(from: "historical_\(date.ISO8601Format()).json"),

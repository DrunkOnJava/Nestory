...
public struct LiveExportService: ExportService {
    ...
    public init() throws {
        fileStore = try FileStore(directory: "exports")
    }
    ...
    public func createBackup(inventory: [Item]) async throws -> BackupPackage {
        ...
        try await fileStore.save(packageData, to: filename)
    ...
    private func createTitlePage(data: ExportData, size: CGSize) -> PDFPage {
        ...
        let subtitle = "Generated on \(Date().formatted())"
        let summary = "\(data.items.count) items • Total value: \(formatCurrency(data.totalValue))"

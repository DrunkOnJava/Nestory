...
public protocol PhotoIntegrationService {
...
public struct LivePhotoIntegrationService: PhotoIntegrationService {
    private let imageIO: ImageIO
    private let thumbnailer: Thumbnailer
    private let perceptualHash: PerceptualHash
    private let logger = Logger(subsystem: "com.nestory", category: "PhotoIntegration")
    ...
    public func processPhoto(_ image: UIImage) async throws -> ProcessedPhoto {
    ...
    public func generateThumbnail(from image: UIImage, size: CGSize) async throws -> UIImage {

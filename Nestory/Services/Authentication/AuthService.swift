...
public struct LiveAuthService: AuthService {
    ...
    public func authenticate() async throws -> AuthCredentials {
        ...
        if hasBiometric {
            if let credentials = try await keychain.load(key: "credentials", type: AuthCredentials.self) {
                return credentials
            }
        }
    ...
    public func signOut() async throws {
        try await keychain.delete(key: "credentials")
    ...
    public func refreshToken(_ token: String) async throws -> AuthCredentials {
        ...
        try await keychain.save(key: "credentials", value: newCredentials)
    ...
    public func currentUser() async -> AuthCredentials? {
        try? await keychain.load(key: "credentials", type: AuthCredentials.self)
    }
...
private class AuthControllerDelegate: NSObject, ASAuthorizationControllerDelegate {
    var continuation: CheckedContinuation<AuthCredentials, Error>?
...
public struct AuthCredentials: Codable, Equatable {

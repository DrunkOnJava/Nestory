...
public struct SettingsFeature {
    ...
    public enum Action {
        ...
        case exportFormatChanged(State.ExportFormat)
        ...
        Reduce { state, action in
            ...
            case let .notificationPermissionResponse(granted):
                ...
                if !granted {
                    ...
                    state.alert = AlertState {
                        ...
                        ButtonState(action: .send(.alert(.dismiss))) {
                            TextState("OK")
                        }
                    } message: {
            ...
            case let .backupFailed(error):
                ...
                state.alert = AlertState {
                    ...
                    ButtonState(action: .send(.performManualBackup)) {
                        TextState("Retry")
                    }
                    ButtonState(action: .send(.alert(.dismiss))) {
                        TextState("Cancel")
            ...
            case .showResetAlert:
                ...
                state.alert = AlertState {
                    ...
                    ButtonState(role: .destructive, action: .send(.confirmReset)) {
                        TextState("Reset")
                    }
                    ButtonState(role: .cancel, action: .send(.alert(.dismiss))) {
                        TextState("Cancel")
                    }
                } message: {

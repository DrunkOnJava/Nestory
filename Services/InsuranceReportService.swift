...
public final class InsuranceReportService: ObservableObject {
    ...
    public enum ReportError: LocalizedError {
        ...
        public var errorDescription: String? {
            switch self {
            case .noItems:
                return "No items to include in report"
            case .pdfGenerationFailed:
                return "Failed to generate PDF report"
            case .dataAccessError:
                return "Could not access inventory data"
            }
        }
    }
    ...
    private func drawSummary(
        ...
        let summaryItems = [
            ...
            "Items with Photos: \(items.filter { $0.imageData != nil }.count)",
            "Items with Serial Numbers: \(items.filter { $0.serialNumber != nil }.count)"
        ]

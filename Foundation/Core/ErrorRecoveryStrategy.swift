...
public protocol ErrorRecoveryStrategy: Sendable {
    ...
    func recover(from error: ServiceError) async throws

    /// Provides fallback data when recovery is not possible
    func fallback<T>(for operationType: String, defaultValue: T) async -> T
...
public struct NetworkRecoveryStrategy: ErrorRecoveryStrategy, Sendable {
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.drunkonjava.nestory.dev", category: "NetworkRecovery")

    public init() {}

    public func recover(from error: ServiceError) async throws {
        switch error {
    ...
    public func fallback<T>(for operationType: String, defaultValue: T) async -> T {
...
public struct AuthenticationRecoveryStrategy: ErrorRecoveryStrategy, Sendable {
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.drunkonjava.nestory.dev", category: "AuthRecovery")
...
public struct CloudKitRecoveryStrategy: ErrorRecoveryStrategy, Sendable {
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.drunkonjava.nestory.dev", category: "CloudKitRecovery")
    ...
    private func resolveSyncConflict(details: String) async throws {
        // Simple conflict resolution - in production, implement proper merge logic
        logger.info("Resolving sync conflict: \(details)")
        try await Task.sleep(nanoseconds: 1_000_000_000) // 1 second
    }
}
...
public struct FileSystemRecoveryStrategy: ErrorRecoveryStrategy, Sendable {
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.drunkonjava.nestory.dev", category: "FileSystemRecovery")
    ...
    private func attemptFileRepair(path: String) async throws {
        logger.info("Attempting file repair for \(path)...")
...
public struct CompositeRecoveryStrategy: ErrorRecoveryStrategy, Sendable {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.drunkonjava.nestory.dev", category: "CompositeRecovery")
    ...
    public func fallback<T>(for operationType: String, defaultValue: T) async -> T {
        for strategy in strategies {
            ...
            return await strategy.fallback(for: operationType, defaultValue: defaultValue)
        }
...
public actor ResilientOperationExecutor {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.drunkonjava.nestory.dev", category: "ResilientExecutor")
    ...
    public func execute<T: Sendable>(
        ...
        do {
            ...
            if recoveryStrategy.canHandle(error) {
                do {
                    try await recoveryStrategy.recover(from: error)
                    ...
                    if let fallbackValue {
                        ...
                        return await recoveryStrategy.fallback(for: operationType, defaultValue: fallbackValue)
                    }

...
public final class CurrencyRate {
    ...
    public init(
        ...
        expiresAt = calendar.date(byAdding: .hour, value: 24, to: asOf)
    ...
    public func extendExpiration(by hours: Int) {
        guard let currentExpiration = expiresAt else {
            let calendar = Calendar.current
            expiresAt = calendar.date(byAdding: .hour, value: hours, to: Date())
            return
        }

        let calendar = Calendar.current
        expiresAt = calendar.date(byAdding: .hour, value: hours, to: currentExpiration)
        updatedAt = Date()
    }
    ...
    public func markAsManual() {
        source = "manual"
        expiresAt = nil // Manual entries don't expire
        updatedAt = Date()
    }
...
public extension CurrencyRate {
    ...
    static func supportedCurrencies(in context: ModelContext) -> Set<String> {

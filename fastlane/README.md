...
### testflight_with_validation

```sh
[bundle exec] fastlane testflight_with_validation
```

Upload current archive to TestFlight with quality checks

### comprehensive_ios_pipeline

```sh
[bundle exec] fastlane comprehensive_ios_pipeline
```

Run comprehensive iOS automation pipeline beyond just uploading

### swiftlint_quality

```sh
[bundle exec] fastlane swiftlint_quality
```

Run SwiftLint code quality analysis with auto-fixes

### simulator_control

```sh
[bundle exec] fastlane simulator_control
```

Control iOS simulators for comprehensive testing

### simulator_cleanup

```sh
[bundle exec] fastlane simulator_cleanup
```
...
```sh
[bundle exec] fastlane semantic_versioning
```

Generate semantic version and comprehensive changelog

### focused_testflight

```sh
[bundle exec] fastlane focused_testflight
```

Upload to TestFlight with focused validation

### run_tools

```sh
[bundle exec] fastlane run_tools
```

Run specific specialized iOS tools individually
...
### ios upload_current_archive

```sh
[bundle exec] fastlane ios upload_current_archive
```

Upload current September 2025 archive to TestFlight
